...
#######################################################################################
# Calculate variable selection stability for each iteration
# for two datasets, fit model, return AB, I(AB != 0), cohen.kappa
mediation_net_ADMM_NC_VSS<-function(data1, data2,
                                    zero.cutoff = 1e-3, # threshold for defining an active path
                                    lambda = 1, omega = 0, phi = 1,
                                    rho = 1 , rho.increase = FALSE,
                                    tol = 1e-10, max.itr = 10000,
                                    thred = 1e-10,
                                    Sigma1 = NULL, Sigma2 = NULL,
                                    trace = FALSE,
                                    k = ncol(data1$M),
                                    data1_Phi1 = diag(c(0, rep(1, k))), data1_Phi2 = diag(rep(1, k + 1)), 
                                    data2_Phi1 = diag(c(0, rep(1, k))), data2_Phi2 = diag(rep(1, k + 1)),
  ...
  return(list(AB=AB.est, Active=ac.set, kappa=re.kappa$kappa))
}

# lambda and omega are vectors
# should add omega column to the final result
# for two datasets, a series of lambdas or omegas, fit model and return the cohen.kappa
mediation_net_ADMM_NC_VSS_rep<-function(data1, data2, zero.cutoff=1e-3,
  ...
  if(length(lambda)==1) { lambda<-rep(lambda,length(omega)) }
  if(length(omega)==1) {  omega<-rep(omega,length(lambda)) }
...
mediation_net_ADMM_NC_KSC<-function(Z, M, R, zero.cutoff=1e-3, 
                                    n.rep=5,vss.cut=0.1,
                                    lambda=10^c(seq(-3,1,length.out=31)[-1],seq(1,5,length.out=11)[-1]), omega=0,
                                    phi=1,Phi1=NULL,Phi2=NULL,
                                    rho=1,rho.increase=FALSE,
                                    tol=1e-10,max.itr=10000,thred=1e-10,
                                    Sigma1=NULL,Sigma2=NULL,
                                    trace=FALSE,Theta0=NULL,D0=NULL,alpha0=NULL,beta0=NULL) {
  n<-nrow(R)
  k<-ncol(M)
  ...
  for(b in 1:n.rep) {
    ...
    data1<-list(Z=matrix(Z[idx1],ncol=1),M=matrix(M[idx1,],ncol=k),R=matrix(R[idx1],ncol=1))
    data2<-list(Z=matrix(Z[idx2],ncol=1),M=matrix(M[idx2,],ncol=k),R=matrix(R[idx2],ncol=1))
